<x-admin-layout>
  ...
  <div class="w-full md:w-1/2  mx-auto bg-slate-50 p-8 rounded-lg shadow-md">
    ...
    <div class="mt-6 mx-auto">
      <h6 class="w-full border-b text-lg font-semibold text-center pt-5">Manage Roles</h6>      
        @if (!empty($roles))
        <div class=" grid grid-cols-2 py-3">
          {{-- Attaching a role for the user --}}
          <form class="mx-auto" action="{{ route('user.role.attach', $user) }}" method="POST">
            @csrf
            @method('PUT')

            <select class="border border-slate-300 px-2 rounded" name="role" id="role">
          ...
          <form class="mx-auto" action="{{ route('user.role.detach', $user) }}" method="POST">
            @csrf
            @method('PUT')
