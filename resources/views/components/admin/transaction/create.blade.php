<x-admin-layout>
  @section('content')
    <div class="w-full max-w-md mx-auto bg-slate-50 p-8 rounded-lg shadow-md">
      <h2 class="text-2xl font-bold mb-6 text-center">Create New Transaction</h2>
      <form action="{{ route('admin.transaction.store') }}" method="POST" class="space-y-4">
        ...
        {{--  Expenses      --}}
        <div id="expense" class="border-y border-slate-300 py-3 sr-only">
        ...
        {{-- Income        --}}
        <div id="income" class="border-y border-slate-300 py-3 sr-only">
          ...
          <div>
            ...
            <select name="service" id="service"
              ...
              <option value="1">Domestic cleaning</option>
              <option value="2">Deep cleaning</option>
              <option value="3">EoT cleaning</option>
            </select>
            @error('service')
            <p class="text-red-500 text-sm">{{ $message }}</p>
            @enderror
